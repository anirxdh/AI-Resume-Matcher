    ...
    <div className="rounded-lg border border-border bg-card p-6 shadow-sm transition-shadow hover:shadow-md">
      ...
      <div className="mt-4 flex flex-wrap gap-4 text-sm text-muted-foreground">
        ...
        {match.salary_min && match.salary_max && (
          <div className="flex items-center space-x-1">
            ...
            <span>
              {match.salary_currency || '$'}{match.salary_min.toLocaleString()} - {match.salary_currency || '$'}{match.salary_max.toLocaleString()}
            </span>
          </div>
        )}
        {match.job_category && (
        ...
        {match.work_location_type && (
          <div className="flex items-center space-x-1">
            <span>{match.work_location_type}</span>
          </div>
        )}
      </div>
      ...
      {/* Expandable Job Details */}
        <div className="mt-4">
          ...
          className="flex w-full items-center justify-between rounded-md border border-border bg-accent px-4 py-2 text-sm font-medium hover:bg-accent/80 transition-colors"
          >
          <span>{isExpanded ? 'Hide' : 'View'} Job Description</span>
            {isExpanded ? (
          ...
          {isExpanded && (
            <div className="mt-3 space-y-4 rounded-md border border-border bg-muted/30 p-4 text-sm">
              {match.responsibilities && (
                ...
                <div 
                  ...
                  dangerouslySetInnerHTML={{ __html: match.responsibilities }}
                />
                </div>
              )}
              {match.requirements && (
            ...
            {/* Additional Job Info */}
            {(match.location || match.work_location_type || (match.salary_min && match.salary_max) || match.h1b_sponsorship !== undefined) && (
              <div className="grid grid-cols-2 gap-4 pt-3 border-t border-border">
                ...
                {match.work_location_type && (
                  <div>
                    ...
                    <p className="text-muted-foreground">{match.work_location_type}</p>
                  </div>
                )}
                {match.salary_min && match.salary_max && (
                  <div>
                ...
                {match.h1b_sponsorship !== undefined && (
                  <div>
                    <span className="font-medium">H1B Sponsorship:</span>
                    <p className="text-muted-foreground">{match.h1b_sponsorship ? 'Yes' : 'No'}</p>
                  </div>
                )}
            </div>
          )}
